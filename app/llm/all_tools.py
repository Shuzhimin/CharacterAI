...
class BaseTool:
    ...
    @Tool(
        name="load_tool",
        description="加载工具",
        params={"tool_name": {"type": "string", "description": "工具名称"}},
    ...
    @staticmethod
    @Tool(
        name="list_tools", description="列出所有可用工具", params={}, required_params=[]
    )
    def list_tools():
...
# 在此处添加你想加入的工具
@Tool(
    name="get_data", description="用于获取角色数据信息", params={}, required_params=[]
)
def get_data():
    data = [
...
@Tool(
    ...
    params={
        "data": {
            "type": "Dict",
            "description": '角色数据信息，输入参数的结构为：{"类别1"：数量1, "类别12"：数量2,...}',
        }
    },
